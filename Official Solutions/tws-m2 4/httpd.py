...
class HTTPServer:
    def __init__(self, IP, port):
        super().__init__()
        self.enable_directory_browsing = False
        self.DOC_ROOT = "/Sankar/2021/ClassOf2021/Official Solutions/tws-m2 4/www"
    ...
    def get_data(self, uri):
        if (uri == "/favicon.ico"):
            ...
            elif(os.path.isfile(self.DOC_ROOT + uri)):
                mime = mimetypes.MimeTypes().guess_type(self.DOC_ROOT + uri)[0]
                mime = str(mime)

                f = open(self.DOC_ROOT + uri, "rb")
                file_data = f.read()
                f.close()
                return 200, mime, len(file_data), file_data
            else:
    ...
    def response_headers(self, status_code, content_type, length):
        ...
        headers += line
        # headers = headers.encode() + data
        return headers
    
    def get_uri(self, http_request):
